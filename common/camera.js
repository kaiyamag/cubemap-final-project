...
class PerspectiveCamera {
    ...
    */
    ...
    /*
    get localRight(){ return new V3(this.cameraMatrix[0],this.cameraMatrix[1],this.cameraMatrix[2]);}
    get localUp(){    return new V3(this.cameraMatrix[4],this.cameraMatrix[5],this.cameraMatrix[6]);}
    get localForward(){    return new V3(this.cameraMatrix[8],this.cameraMatrix[9],this.cameraMatrix[10]);}

    update(){
        let speed = 4;
        var newRotation;
        if(Input.keyHold("arrowup")){
            let forward = this.localForward;
            forward.multiplyScalar(deltaTime * speed); // scale according to speed and frameRate
            this.setPositionRotation(getPosition().addV(forward), getViewDir())
        }
        if(Input.keyHold("arrowdown")){
            let forward = personBody.localForward;
            forward.multiplyScalar(deltaTime * speed); // scale according to speed and frameRate
            this.setPositionRotation(getPosition().addV(forward), getViewDir())
        }
        if(Input.keyHold("arrowleft")){
            newRotation = getViewDir();
            newRotation.y += deltaTime * 270;
            this.setPositionRotation(getPosition(), newRotation)
        }
...
class OrbiterCameraController {
    ...
    */
    constructor(canvas, camera, target, distance, zoomSpeed, rotationSpeed, movementSpeed, startingPitch, tartingYaw){
        ...
        this.offsetX = 0;
        this.offsetZ = 0;
        this.panSpeed = 2;  // Speed of camera rotation
        this.movementSpeed = 0.2;   // Speed of camera forward movement
    }
    ...
    */
    update(){
        var forward = new V3(this.target.x, this.target.y, this.target.z);
        ...
        if(Input.keyHold("arrowup")){
            // Update x, y, z position with new position if camera moves one unit forward in the direction its facing
            forward.set(this.camera.cameraMatrix[8],this.camera.cameraMatrix[9],this.camera.cameraMatrix[10]);
            this.offsetX -= forward.x * this.movementSpeed;
            this.offsetY -= forward.y * this.movementSpeed;
            this.offsetZ -= forward.z * this.movementSpeed;
        }
        if(Input.keyHold("arrowdown")){
            forward.set(this.camera.cameraMatrix[8],this.camera.cameraMatrix[9],this.camera.cameraMatrix[10]);
            this.offsetX += forward.x * this.movementSpeed;
        ...
        this.camera.cameraMatrix = M4.translationMatrix(this.target.x + this.offsetX, this.target.y + this.offsetY, this.target.z + this.offsetZ);
