...
// ADDITION
uniform float u_time; // the current time, used to calculate animations
...
void main() {
    ...
    // Add swaying
    vec3 pos = a_position;
    //pos.x += sin(u_time + pos.y) * 0.1 * (pos.y + 0.8);
    pos.x += sin(u_time) * (pos.y + 0.8) * 0.1;
    //pos.x += 100.0 * sin(u_time);
    // PROBLEM: can adjust position by a constant (fx. 1.0), but nothing to do with u_time...
    // Is u_time being passed in correctly? How do I debug that?
